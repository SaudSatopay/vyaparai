...
- **Qwen Cloud (`qwen-plus`)** — the agent, the classifier, the translator.
- **Alibaba Cloud** — the app is deployed on Function Compute / ECS.
...
Live IRP-sandbox integration for a genuine signed IRN and QR, the WhatsApp Cloud API as
a real channel, UPI payment links, and a two-way clarification loop with the customer.
